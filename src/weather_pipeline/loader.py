...
class WeatherLoader:
    ...
    def execute_schema_script(self, schema_path: str) -> None:
    ...
    def load_dim_location(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def load_dim_date(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def load_fact_weather(self, df: pd.DataFrame, location_map: pd.DataFrame, date_map: pd.DataFrame) -> None:
        ...
        with self.engine.connect() as conn:
            for _, row in fact_df.iterrows():
                ...
                """)
                exists = conn.execute(check_sql, {"loc_id": row["location_id"], "date_id": row["date_id"]}).fetchone()
